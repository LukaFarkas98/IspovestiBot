...
# Modal App
# -----------------------
app = modal.App("aya23-confessions")
...
def train_confessions_model():
    OUTPUT_DIR = f"/mnt/models/aya23-confessions-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    def tokenize(example):
        tokens = tokenizer(
            example["text"],  # prefix already added in dataset
            truncation=True,
            max_length=MAX_LEN,
            padding="max_length",
        )
